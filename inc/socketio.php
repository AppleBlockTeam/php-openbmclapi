...
class socketio {
    ...
    public function connect() {
        ...
        $query = '/socket.io/?EIO=4&transport=websocket';
        if ($this->sid) {
            $query .= '&sid=' . $this->sid;
        }
        $ret = $client->upgrade($query);
    ...
    private function reconnect() {
        $retryCount = 0;
        while (!$this->Connected && $retryCount < 3) {
            mlog("重连尝试 " . ($retryCount + 1));
            $this->connect();
            $retryCount++;
            if (!$this->Connected) {
                Coroutine::sleep(2); // 重试前等待2秒
            }
        }
